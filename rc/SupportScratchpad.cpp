...
class MyCFGDumper : public Checker<check::ASTCodeBody> {
  ...
  private:
    ...
    void handleDeclStmt(std::stack<const Stmt *> &helper_stack, InstructionListTy &instr_list,
    ...
    void handleAssignment(std::stack<const Stmt *> &helper_stack, InstructionListTy &instr_list,
                          int &temp_counter, unsigned &block_id) const;

    std::string reduce(std::stack<const Stmt *> &helper_stack, InstructionListTy &instr_list,
                       int &temp_counter, unsigned &block_id, BehaviorTy behavior) const;

    std::string getOperandWithSideEffects(std::string operand, const UnaryOperator *un_op,
    ...
    void handleTerminator(const Stmt *terminator, std::stack<const Stmt *> &helper_stack,
                          int &temp_counter, unsigned &block_id) const;
        ...
        for (unsigned i = 0, e = func_decl->getNumParams(); i != e; ++i) {
            ...
            QualType T =
                varDecl->getTypeSourceInfo()
                    ...
                    : varDecl->getASTContext().getUnqualifiedObjCPointerType(varDecl->getType());
            Proto += T.getAsString();
...
void MyCFGDumper::handleDeclStmt(std::stack<const Stmt *> &helper_stack,
    ...
    current_instr_stream << T.getAsString() << " " << named_decl->getNameAsString() << " = "
                         << reduce(helper_stack, instr_list, temp_counter, block_id,
                                   REDUCE_EXPR_TO_THREE_ADDR);
    instr_list.push_back(current_instr_stream.str());
}

std::string MyCFGDumper::getReducedTemporary(std::stack<const Stmt *> &helper_stack,
                                             InstructionListTy &instr_list, int &temp_counter,
                                             unsigned &block_id) const {
    ...
    std::string left_operand =
        getReducedTemporary(helper_stack, instr_list, temp_counter, block_id);
    current_instr_stream << " = " << left_operand << " " << std::string(bin_op->getOpcodeStr())
                         << " " << right_operand;
...
// getReducedTemporary
std::string MyCFGDumper::getDevelopedRValue(std::stack<const Stmt *> &helper_stack,
                                            InstructionListTy &instr_list, int &temp_counter,
                                            unsigned &block_id) const {
    ...
    ++temp_counter;
    operand = getReducedTemporary(helper_stack, instr_list, temp_counter, block_id);

    operand = getOperandWithSideEffects(operand, un_op, instr_list, temp_counter, block_id);
...
std::string MyCFGDumper::getPrimitiveValue(std::stack<const Stmt *> &helper_stack) const {
    const Stmt *current_stmt = helper_stack.top();
    helper_stack.pop();
...
std::string MyCFGDumper::getOperandWithSideEffects(std::string operand, const UnaryOperator *un_op,
                                                   InstructionListTy &instr_list, int &temp_counter,
                                                   unsigned &block_id) const {
    if (!un_op)
        return operand;
...
std::string MyCFGDumper::reduce(std::stack<const Stmt *> &helper_stack,
    ...
    if (isa<BinaryOperator>(current_stmt) && behavior == REDUCE_EXPR_TO_THREE_ADDR) {
        const BinaryOperator *bin_op = cast<BinaryOperator>(current_stmt);
        helper_stack.pop();
        ...
        current_instr_stream << getReducedTemporary(helper_stack, instr_list, temp_counter,
                                                    block_id);
    }
...
void MyCFGDumper::handleAssignment(std::stack<const Stmt *> &helper_stack,
    ...
    if (isa<UnaryOperator>(helper_stack.top())) {
        un_op = const_cast<UnaryOperator *>(cast<UnaryOperator>(helper_stack.top()));
        helper_stack.pop();
    }

    LHS = reduce(helper_stack, instr_list, temp_counter, block_id, REDUCE_EXPR_TO_SINGLE_TEMP);

    // in this case, only possible unary operators are * and &
    LHS = getOperandWithSideEffects(LHS, un_op, instr_list, temp_counter, block_id);
    current_instr_stream << LHS << " = ";
    current_instr_stream << reduce(helper_stack, instr_list, temp_counter, block_id,
                                   REDUCE_EXPR_TO_THREE_ADDR);
    instr_list.push_back(current_instr_stream.str());
}
